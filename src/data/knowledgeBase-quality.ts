...
export const qualityKB: KBEntry[] = [
  {
    ...
    faqs: [
      ...
      { q: 'What is the difference between QA and QC in pharma?', a: 'Quality Control (QC) focuses on testing and inspection — laboratory analysis of raw materials, in-process samples, and finished products to determine if they meet specifications. Quality Assurance (QA) focuses on systems and oversight — ensuring the processes that produce and test products are controlled and validated. QC is a component of QA. QC reports to QA; QA provides the authority for batch release (through QP in the EU or QA Director in the US). Both functions are required under GMP regulations.' },
